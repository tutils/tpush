...
import (
	...
	"github.com/tutils/tpush/options"
	"github.com/tutils/tpush/srv/push/handler"
	push "github.com/tutils/tpush/srv/push/proto/push"
	"github.com/tutils/tpush/srv/push/subscriber"
...
func main() {
	...
	service.Init(
		micro.Action(func(c *cli.Context) error {
			...
			if f := c.String("recv_timeout"); len(f) > 0 {
				chatroom.RecvTimeout = time.Duration(float64(time.Second) * c.Float64("recv_timeout"))
			}

			if f := c.String("login_timeout"); len(f) > 0 {
				chatroom.LoginTimeout = time.Duration(float64(time.Second) * c.Float64("login_timeout"))
			}

			if f := c.String("stream_pattern"); len(f) > 0 {
				chatroom.StreamPattern = f
			}
	...
	if enable_distribute {
		...
		d := chatroom.NewEtcdDistribute(nodeId, c, time.Second*30)
		d.Run()

		opts = append(opts, chatroom.WithDistribute(d))
	}

	service2 := chatroom.NewService(opts...)

	service2Done := make(chan struct{})

	go func() {
		// 启动web服务
		log.Infof("Server [web] Listening on %s", chatroom.Address)
